...
# CONTENTS:
# 1. function PlotSum
# 2. function GrPlot (updated 28 June 2023)
# 3. function PlotsD
# 4. function PlotEO
# 5. function getdats
#-------------------------------------------------------------------------------

# Updates 28 June 2023: Gradient plots updated to show fishers Z transformed correlations
...
GrPlot = function(parm="TOC", start = T, row = 1, textpos = -1, xlow = -1, xhigh = 1, ylow = 0, yhigh = 2,
  ...
  # Calculations for Fishers Z transformed correlation
  ff = tt[is.finite(tt$V),]                                # remove rows with infinate values 
  ll = rma(yi = ff$r, vi = ff$V)                           # Use rma function to get tau^2 and other values
  T2 = ll$tau2
  ff$WRand = 1/ff$W + T2      # Weight = 1/variance within + variance between studies
  ff$WYRand = ff$WRand * ff$r
  
  M  = round((sum(ff$WYRand, na.rm = T)/sum(ff$WRand, na.rm = T)), digits = 2) # summary EF calculation
  Vm = 1/(sum(ff$WRand, na.rm = T))                          # variance
  SD = sd(ff$r, na.rm = T)                                   # Standard deviation of r in subset
  SE = sqrt(Vm)
  LL = round(M - 1.96 * SE, digits = 2)
  UL = round(M + 1.96 * SE, digits = 2)
  ...
  if (start == TRUE) {
    ...
    mtext("SMD [95% CI]", side = 3, at = xhigh-5, font = 3, cex = 0.75)
