...
library(gridExtra)
library(maps)
source("../source_code/FIN_functions_meta_analysis_reviewed.R")
...
GradInfo$Response.variable[GradInfo$Response.variable == "OM (LOI)"] <- "TOC"

load(file = "../r_objects/SummaryEF_Gradients_new.rda")
...
fulldfsel <- names(dfshort)[c(8,9,15,35,36,37,20,2,1,14,18,19)]
colnames(gradsel) <- fulldfsel

gradsel$hhabtype <- ifelse(gradsel$hhabtype == "Shallow sandy", "Sand", ifelse(gradsel$hhabtype == "Shallow sandy, low tidal disturbance", "Sand", "Mud"))
gradsel$hseason[gradsel$hseason == ""] <- "Summer" 
gradsel$hseason[gradsel$hseason == "Summer, rainy season, upwelling"] <- "Summer"
gradsel$hseason[gradsel$hseason == "temperate latitude, seasonal, rough seas mentioned in text"] <- "Summer" 

gradsel$gear[gradsel$gear == "otter trawl"] <- "Otter Trawl"
gradsel$gear[gradsel$gear == "Mussel dredge"] <- "Dredging"
gradsel$gear[gradsel$gear == "scallop dredge"] <- "Dredging"
gradsel$gear[gradsel$gear == "Scallop"] <- "Dredging"
gradsel$gear[gradsel$gear == "mixed fishing gears"] <- "Mixed"
gradsel$gear[gradsel$gear == "otter trawl beam trawl"] <- "Otter Trawl"
gradsel$gear[gradsel$gear == "Lobster"] <- "Local gear net"
gradsel$gear[gradsel$gear == ""] <- "Otter Trawl"
gradsel$gear[gradsel$gear == "Otter trawl"] <- "Otter Trawl"

gradsel$respvar[gradsel$respvar == "Silt  "] <- "Silt"

gradsel$slicedepth[gradsel$slicedepth == "surface sediment from Van Veen Grabs"] <- "surface sediment"
gradsel$slicedepth[gradsel$slicedepth == "0"] <- "surface sediment"
gradsel$slicedepth[gradsel$slicedepth == ""] <- "surface sediment"
...
world <- ggplot() +
  ...
  geom_point(data = datall, aes(x = Longitude_study, y = Latitude_study), fill = "orange", size = 3, shape = 21) +
...
studytype2 <- studytype
studytype2$`datall$hstype`[studytype2$`datall$hstype` == "Gradient"] <- "Comparative gradient"
studytype2 <- studytype2 %>%
  mutate(studytype_ordered = factor(studytype2[,1], 
                                    levels = names(sort(table(studytype2[,1]), 
                                                        decreasing = TRUE))))

studyplot <- ggplot(data = studytype2, aes(x = studytype_ordered)) + 
...
## E. Historically fished

dfshort$CTRLhisttrawledplot         <- as.factor(dfshort$CTRLhisttrawled)
levels(dfshort$CTRLhisttrawledplot) <- c("No", "Yes", "No information")
dfshort$CTRLhisttrawledplot[is.na(dfshort$CTRLhisttrawledplot)] <- "No information"
gra           <- df[,c(6, 1, 5)]
colnames(gra) <- colnames(histfishedsplit)
gra           <- gra[c(1:6,6,7,7,8,8,9:14),]
gra$`dfshort$article_type_id`[c(7,9,11)] <- c(41.1, 49.1, 53.)

histfished <- dfshort %>% distinct(dfshort$CTRLhisttrawledplot, dfshort$article_type_id)
histfished <- rbind(histfished, gra[,c(1,2)])
histfished$`dfshort$CTRLhisttrawledplot`[histfished$`dfshort$CTRLhisttrawledplot` == "no"]  <- "No"
histfished$`dfshort$CTRLhisttrawledplot`[histfished$`dfshort$CTRLhisttrawledplot` == "yes"] <- "Yes"
histfished$`dfshort$CTRLhisttrawledplot` <- factor(histfished$`dfshort$CTRLhisttrawledplot`, levels = c("Yes", "No", "No information"))

histfishedplot <- ggplot(data = histfished, aes(x = `dfshort$CTRLhisttrawledplot`)) + 
                  geom_bar(width = bar_width, position = position_dodge(width = 1), fill = "lightgrey") +
                  # geom_text(stat='count', aes(label= after_stat(count)), vjust=-1) + 
                  theme_classic() + 
                  # ylim(0, 45) +
                  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
                  #theme(plot.title = element_text(hjust = 0.5)) + 
                  labs(title="E  Historic fishing", y = "", x = " ")

histfishedplot

# Combine the plots using patchwork
combined_plots <- seasonplot + studyplot + depthplot + world + histfishedplot
design <- "44444444444444
           44444444444444
           44444444444444
           44444444444444
           33333221111155
...
combined_plots
ggsave("../final_review/figure1_info_world_season_stype_depth.png", units = "in", height = 10, width = 12)

```

#### Figure1Supplement: Justin edits

```{r label, options}

habitat2 <- datall %>% distinct(datall$hhabtype, datall$article_type_id, datall$hstype) #JT

gear2               <- datall %>% distinct(datall$gear, datall$article_type_id, datall$hstype) #JT)
gear2$`datall$gear` <- factor(gear2$`datall$gear`, levels= c(unique(gear2$`datall$gear`)[c(1,4,2,3,6,7,5)]))

comz2             <- datall %>% distinct(datall$samplestrat, datall$article_type_id, datall$hstype) # JT modified
names(comz2)      <- c("Samplestyle","ArticleID",  "stype")
...
stratplot <- ggplot(data = comz2, aes(x = Samplestyle)) + 
             ...
             theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
             #theme(plot.title = element_text(hjust = 0.5)) + 
             labs(title= "C  Sampling method", y = "No. studies", x = " ") +
             facet_wrap(~ comz2$stype, ncol = 1)  # 


## Sediment type
sedimentplot <- ggplot(data = habitat2, aes(x = habitat2$`datall$hhabtype`)) + 
                geom_bar(width = bar_width, position = position_dodge(width = 1), fill = "lightgrey") +
                # geom_text(stat='count', aes(label= after_stat(count)), vjust=-1) + 
                theme_classic() + 
                # ylim(0, 45) +
                theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
                #theme(plot.title = element_text(hjust = 0.5)) + 
                labs(title="A  Sediment type", y = "", x = " ") +
                facet_wrap(~ habitat2$`datall$hstype`, ncol = 1)  # JT: Separate the plot into panels based on the 3rd column

## Gear plot
gearplot <- ggplot(data = gear2, aes(x = gear2[,1])) + 
            geom_bar( width = bar_width, position = position_dodge(width = 1), fill = "lightgrey") +
            # geom_text(stat='count', aes(label= after_stat(count)), vjust=-1) + 
            theme_classic() +
            #ylim(0, 44) +
            theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
            #theme(plot.title = element_text(hjust = 0.5)) + 
            labs(title="B  Gear type", y = "", x = " ")+
            facet_wrap(~ gear2$`datall$hstype`, ncol = 1)  

## Historically fished
histfishedsplit <- dfshort %>% distinct(dfshort$CTRLhisttrawledplot, dfshort$article_type_id, dfshort$hstype)
...
histfishedsplit <- rbind(histfishedsplit, gra)
histfishedsplit$`dfshort$CTRLhisttrawledplot`[histfishedsplit$`dfshort$CTRLhisttrawledplot` == "no"] <- "No"
histfishedsplit$`dfshort$CTRLhisttrawledplot`[histfishedsplit$`dfshort$CTRLhisttrawledplot` == "yes"] <- "Yes"
histfishedsplit$`dfshort$CTRLhisttrawledplot` <- factor(histfishedsplit$`dfshort$CTRLhisttrawledplot`, levels = c("Yes", "No", "No information"))


histfishedsplitplot <- ggplot(data = histfishedsplit, aes(x = histfishedsplit[,1])) + 
                       geom_bar( width = bar_width, position = position_dodge(width = 1), fill = "lightgrey") +
                       # geom_text(stat='count', aes(label= after_stat(count)), vjust=-1) + 
                       theme_classic() +
                       #ylim(0, 44) +
                       theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
                       #theme(plot.title = element_text(hjust = 0.5)) + 
                       labs(title="D  Historically fished", y = "", x = " ")+
                       facet_wrap(~ histfishedsplit[,3], ncol = 1)  

histfishedsplitplot

# Combine the plots using patchwork
combined_plots <- sedimentplot +  gearplot + stratplot + histfishedsplitplot
design <- "11223344
           11223344
"
# Set the heights of the plots to be equal
combined_plots <- combined_plots +
  plot_layout(guides = "collect", heights = c(1, 1, 1, 1), design=design)

# Display the combined plots
combined_plots

ggsave("../final_review/figure1SUPPLEMENT_info_strategy_stype_geartype.png", units = "in", height = 10, width = 12)
...
ggsave("../final_review/figure1SUPPLEMENT_info_strategy_stype_geartype.png", units = "in", height = 6, width = 12)
...
ggsave("../final_review/SUPPLEMENT_samples_per_variable_dfshort.png", units = "in", height = 5, width = 9)
...
ggsave("../final_review/SUPPLEMENT_samples_per_variable_expobs_morethan3studies.png", units = "in", height = 5, width = 9)
...
ggsave("../final_review/SUPPLEMENT_samples_per_variable_gradients.png", units = "in", height = 5, width = 9)
...
ggsave("../final_review/SUPPLEMENT_samples_per_variable_gradient_morethan3studies.png", units = "in", height = 5, width = 9)
...
ggsave("../final_review/SUPPLEMENT_variables_allstudytypes_morethan3studies.png", units = "in", height = 12, width = 9)
...
variables <- variables[c(2,5,7,1,4,3,14, 6,10,9,13,17,15,12,8,18,19,20,11)]
...
p <- ggplot(data = suball, aes(y = factor(v, levels = v))) +
     ...
     coord_cartesian(ylim=c(1,20), xlim=c(-1.8, 1.0)) +  
     theme_classic() +
     annotate("text", x = -0.5, y = 20, label = "Decrease", size = 3.5) +
     annotate("text", x = 0.5, y = 20, label = "Increase", size = 3.5) + 
...
pdf("../final_review/Figure2_trueresponseCORRECTED.pdf", height = 8, width = 10) # JT
pleft + p + pright + plot_layout(design = layout)
dev.off()
...
dfsummed2 <- summarizer2b(dfshort)

suball2    <- dfsummed2[dfsummed2$v %in% variables,]
variables2 <- rep(variables, each = 2)
# variables2 <- variables2[-c(23, 24)]

# Making sure the variables will be displayed as desired
suball2   <- suball2[order(match(suball2$v, variables2)),]  
suball2   <- suball2[nrow(suball2):1,]
suball2$v <- factor(suball2$v, levels = variables)
 
## Preparing plot
p <- ggplot(data = suball2, aes(y = rev(v), shape = stype)) +
     ...
     coord_cartesian(ylim=c(1,20), xlim=c(-2.5, 2)) +  
     theme_classic() +
...
# subover2 <- subover2[nrow(subover2):1,]
...
subover2$v  <- factor(subover2$v , levels = c(rev(levels(subover2$v)[1:19]), "Variable"))
# 
subover2$ns <- factor(subover2$ns, levels = c(rev(subover2$ns)[1:37], "n Obs. (studies)"))
...
pleft <- ggplot(data = subover2, aes(y = v)) +
         ...
         theme_void() +
         #scale_fill_manual(values=pcolz) + # modified
         coord_cartesian(xlim = c(0, 4), ylim=c(1,20))
...
pdf("../final_review/Figure2SUPPLEMENT_expobs_separated_CORRECTED.pdf", height = 8, width = 10) # JT
pleft + p + pright + plot_layout(design = layout)
dev.off()

png("../final_review/Figure2SUPPLEMENT_expobs_separated_CORRECTED.png", height = 8, width = 10, units = "in", res = 330) # JT
pleft + p + pright + plot_layout(design = layout)
dev.off()
...
gradplot <- data.frame(v = sumData2$Variable, M = sumData2$M, LL = sumData2$LowerBound, UL = sumData2$UpperBound, pval = NA, n = sumData2$Observations, s = sumData2$Studies)

gradplot$estimate_lab <- paste0(gradplot$M, " [", gradplot$LL, " - ", gradplot$UL, "]")
gradplot$ns           <- paste0(gradplot$n, " (", gradplot$s, ")")

gradplot$v <- reorder(gradplot$v, gradplot$M) # JT
gradplot   <- gradplot[c(2,3,4,1,5),]
gradplot$v <- factor(gradplot$v, levels = rev(c("Chl-a", "TOC", "TN", "Mean grain size", "Mud content (< 63 µm)")))
...
p <- ggplot(data = gradplot, aes(y = v)) +
     ...
     coord_cartesian(ylim=c(1,6), xlim=c(-0.9, 2)) +  
     theme_classic() +
     annotate("text", x = -0.5, y = 6, label = "Decrease", size = 3.5) +
     annotate("text", x = 0.5, y = 6, label = "Increase", size = 3.5) + 
     theme(axis.line.y = element_blank(),
           axis.ticks.y= element_blank(),
           axis.text.y= element_blank(),
           axis.title.y= element_blank())
...
pleft <- ggplot(data = subover2, aes(y = v)) +
         ...
         theme_void() +
         coord_cartesian(xlim = c(0, 4), ylim=c(1, 6))
...
pdf("../final_review/Figure2_grad_trueresponseJT.pdf", height = 2.5, width = 10) #JT
pleft + p + pright + plot_layout(design = layout)
dev.off()
...
for(i in unique(dfshort$respvar)){
  
  subset <- dfshort[dfshort$respvar == i,]
  print(i)
  
  for(a in unique(subset$article_type_id)){
    
    subset2 <- subset[subset$article_type_id != a, ]
    
    if(length(subset2$respvar) <= 1){
      print(paste0("for ", i, " not enough studies"))
    } else{
      
    subset2 <- escalc(data = subset2, yi = lnRR, vi = VarLnRR) # Just transforming in escalc object
    varcov  <- vcalc(vi = vi, cluster = article_type_id, data = subset2, nearpd = TRUE) # cluster variances in variance-covariance matrix per study type for use in aggregate.
    subset2 <- aggregate(x = subset2, cluster = article_type_id, V = varcov, struct = "CS", weighted = FALSE) # aggregate effect sizes in single study effect size.
    outp    <- rma(data = subset2, yi = yi, vi = vi)
    outp    <- c("estimate" = outp$beta,
                 "se"       = outp$se,
                 "zval"     = outp$zval,
                 "pval"     = outp$pval, 
                 "ci.lb"    = outp$ci.lb,
                 ...
                 "study"    = a,
                 "variable" = i)
    resdf <- rbind(resdf, outp)
    }
  }
  resdf <- data.frame(resdf)
  resdf[,c(1:6)] <- lapply(resdf[,c(1:6)], function(x) as.numeric(as.character(x)))
}

```

```{r plotleave1out, fig.height = 10, fig.width = 12}
require(ggplot2)
require(patchwork)
auths <- read.csv("../dataset/Authors.csv", sep = ";")
auths <- auths
colnames(auths) <- c("study", "authors")
auths$study     <- as.character(auths$study)

resdf2 <- resdf[resdf$variable %in% variables,]
resdf2 <- resdf2[match(variables, resdf2$variable),]

resdf2 <- resdf2 %>%
  left_join(auths, by = "study")
...
for(i in unique(resdf2$variable)){
  
  testsub <- resdf[resdf$variable == i,]

  truemid      <- dfsummed$M[dfsummed$v == i]
  truelb       <- dfsummed$LL[dfsummed$v == i]
  trueub       <- dfsummed$UL[dfsummed$v == i]
  xpos         <- max(testsub$ci.ub)
  testsub$xpos <- xpos + 0.05
  trueconfint  <- paste0("[", round(testsub$ci.lb,2), " - ", round(testsub$ci.ub, 2), "],")
  truep        <- paste0("p = ", round(testsub$pval, 3))
  testsub$authslab <- paste(paste0(testsub$authors, ":"), trueconfint, truep)
  
  plot <- ggplot(data = testsub, aes(x = estimate, y = authors)) +
          geom_point(shape = 18, size = 3) +
          geom_linerange(aes(xmin = ci.lb, xmax = ci.ub)) +
          geom_vline(xintercept = truemid, linetype="dashed") +
          geom_vline(xintercept = truelb, linetype="dashed", col = "lightgrey") +
          geom_vline(xintercept = trueub, linetype="dashed", col = "lightgrey") +
          geom_vline(xintercept = 0, col = "black") +
          coord_cartesian(clip = 'off') +
          geom_text(aes(x = xpos), label = paste(trueconfint, truep), hjust = 0, size = 3) +  # Add p-values and confidence intervals as text labels
          ylab(" ") + 
          labs(title = i) + 
          theme_minimal() +
...
# Combine --> Uses patchwork
comb1 <- pl[[1]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[2]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) +  
         pl[[3]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[4]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[5]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[6]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt"))

comb2 <- pl[[7]]  + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[8]]  + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[9]]  + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[10]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[11]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[12]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[13]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[14]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[15]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[16]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[17]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[18]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt")) + 
         pl[[19]] + theme(plot.margin = unit(c(0, 100, 0, 0), "pt"))

# Set the heights of the plots to be equal
combsed <- comb1 +
  plot_layout(nrow = 3, ncol = 2, guides = "collect") +
  plot_annotation(tag_levels = 'A')#, heights = c(1, 1, 1, 1),

# Display the combined plots
combsed
ggsave("../final_review/leave1out_a.png", units = "in", height = 14, width = 14)


combsed2 <- comb2 + 
  plot_layout(guides = "collect", nrow = 7, ncol = 2) + 
  plot_annotation(tag_levels = 'A')

combsed2
ggsave("../final_review/leave1out_b.png", units = "in", height = 20, width = 14)

```

# Funnel plots

```{r funnelplots, options}

rtlist <- list()
trimfillist <- list()
tf <- data.frame(matrix(ncol = 11, nrow = 0))

for(i in variables){
  
  subset <- dfshort[dfshort$respvar == i,]
  subset <- subset[subset$ogVarLnRR < 20,]
  
  if(any(subset$ogVarLnRR == 0)){
    subset$ogVarLnRR[subset$ogVarLnRR == 0 ] <- 0.01
  }
  
  outp   <- rma(yi = lnRR, vi = ogVarLnRR, data = subset)
  subset <- escalc(data = subset, yi = lnRR, vi = ogVarLnRR) # Just transforming in escalc object
  varcov <- vcalc(vi = vi, cluster = article_type_id, data = subset, nearpd = TRUE) # cluster variances in variance-covariance matrix per study type for use in aggregate.
  subset <- aggregate(x = subset, cluster = article_type_id, V = varcov, struct = "CS", weighted = FALSE) # aggregate effect sizes in single study effect size.
  outp   <- rma(data = subset, yi = yi, vi = vi)
    
  restf <- trimfill(outp, estimator = "R0")

  rtlist[[i]] <- regtest(outp, model = "rma")
  trimfillist[[i]] <- restf
  
  tf <- rbind(tf, c(restf$k0, restf$side, restf$p.k0,restf$QE, restf$QEp, restf$b, restf$pval, restf$se,  restf$ci.lb, restf$ci.ub,  i))
  
  # c("estmissed" = restf$k0, "side" = restf$side, "pval" = restf$p.k0, "Q" = restf$QE, "pval2" = restf$QEp, "Estimate" = restf$b, "se" = restf$se, "cilb" = restf$ci.lb, "ciub" = restf$ci.ub, "var" = i))
  
  nm <- i
  if(nm == "TOC/TN"){
    nm <- "TOCTN"
  }
  if(nm == "Mud content (< 63 µm)"){
    nm <- "Mudcontent"
  }
  
  png(filename = paste0("../final_review/", nm, "_funnel.png"), units = "in", height = 5, width = 6, res = 330)
  funnel(restf, legend=TRUE, cex=1, main = i, xlab = "Log-response ratio",  level=c(90, 99), shade=c("white", "gray75"), pch  = 20) #, refline=0
  dev.off()
  
}

colnames(tf) <- c("estmissed", "side", "pval", "Q", "pval2", "Estimate", "pval3", "se", "cilb", "ciub", "var")
...
For figure 3, I'm staying with the significant parameters only + TN (to fill up a spot), since another plot would become too large. We can say: "Here are the significant ones, the other non-significant ones are in supplement"

### Averages per depth stratum

Here we use the function "summarizer_varslice". This function calculates the average effect size per depth stratum, per study, for each variable. This will be used in the plot, showing the within-study-means, along with the overall means, per depth slices.
...
tocount    <- table(out$respvar, out$article_type_id,out$depths)
...
write.table(slicemods, file = "../tables_final/slices.txt", sep = ",", quote = FALSE, row.names = F)
...
# Combo
combined_plots <-  chlaplot + phaeplot + tocplot + tnplot + protplot + mgsplot +
...
ggsave("../final_review/figure3_slicedepth_significantparmsandtn.pdf", units = "in", height = 10, width = 8)  
...
ggsave("../final_review/figure3SUPPLEMENT_slicedepth_nonsig.pdf", units = "in", height = 5, width = 15)  
...
mycol = adjustcolor("grey",0.75); mycol2 = adjustcolor("grey",0.5)
...
write.table(toc_mods, file = "../tables_final/toc_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(tn_mods, file = "../tables_final/tn_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(Chla_mods, file = "~../tables_final/Chla_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(phaeo_mods, file = "../tables_final/phaeo_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(phyto_mods, file = "../tables_final/phyto_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(mgs_mods, file = "../tables_final/mgs_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(silt_mods, file = "../tables_final/silt_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(prot_mods, file = "../tables_final/prot_models.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(NHx_mods, file = "../tables_final/NHx_models.txt", sep = ",", quote = FALSE, row.names = F)

```

## Figure 4: Regressions --\> 

Here we select whichever regressions we think are nice and should be shown in the manuscript.
...
pdf("../figures_final/regplots.pdf", height = 3.5, width = 10) #

par(mfrow = c(1,2), mar = c(5,4,2,1))
plotreg(resp = "TOC", depvar = "cvel", df = dfshort, xlabz = "Current velocity", log = FALSE, sample = "Subsurface", title = "Subsurface TOC and Current Velocity")
plotreg(resp = "Chl-a", depvar = "timesincetrawl", df = timedfchla, xlabz = "log (timesincetrawl)", log = TRUE, title = "Surface Chl-a Recovery")
...
## S5: Surface Chla - cvel; subsurface TOC - cvel; subsurface TN - cvel
png("../figures_final/regplot_s5.png", height = 3.5, width = 10, units = "in", res = 330) #

par(mfrow = c(1,3), mar = c(5,4,2,1))
plotreg(resp = "Chl-a", depvar = "cvel", df = dfshort, xlabz = expression(paste("Current velocity (m", " s"^"-1", ")")), log = FALSE, sample = "Surface", title = "Surface Chl-a - current velocity")
plotreg(resp = "TOC", depvar = "cvel", df = dfshort, xlabz =  expression(paste("Current velocity (m", " s"^"-1", ")")), log = FALSE, sample = "Subsurface", title = "Subsurface TOC - current velocity")
plotreg(resp = "TN", depvar = "cvel", df = dfshort, xlabz =  expression(paste("Current velocity (m", " s"^"-1", ")")), log = FALSE, sample = "Subsurface", title = "Subsurface TN - current velocity")

dev.off()


## S6: Surface chla - PP; surface phaeo - PP; surface phyto - PP; surface protein - PP 
png("../figures_final/regplot_s6.png", height = 11, width = 10, units = "in", res = 330) #

par(mfrow = c(3, 2), mar = c(5,4,2,1))

plotreg(resp = "Chl-a", depvar = "nppsurf", df = dfshort, xlabz = expression(paste("Surface primary productivity", " (g", "m"^"-3", "d"^"-1", ")")), log = FALSE, sample = "Surface", title = "Surface Chl-a - surface PP")

plotreg(resp = "Phaeopigments", depvar = "nppsurf", df = dfshort, xlabz =  expression(paste("Surface primary productivity", " (g", "m"^"-3", "d"^"-1", ")")), log = FALSE, sample = "Surface", title = "Surface phaeopigment - surface PP")

plotreg(resp = "Phytopigments", depvar = "nppsurf", df = dfshort, xlabz =  expression(paste("Surface primary productivity", " (g", "m"^"-3", "d"^"-1", ")")), log = FALSE, sample = "Surface", title = "Surface phytopigment - surface PP")

plotreg(resp = "Proteins", depvar = "nppsurf", df = dfshort, xlabz =  expression(paste("Surface primary productivity", " (g", "m"^"-3", "d"^"-1", ")")), log = FALSE, sample = "Surface", title = "Surface proteins - surface PP")

plotreg(resp = "TN", depvar = "nppsurf", df = dfshort, xlabz =  expression(paste("Surface primary productivity", " (g", "m"^"-3", "d"^"-1", ")")), log = FALSE, sample = "Subsurface", title = "Subsurface TN - surface PP")

plotreg(resp = "TN", depvar = "nppsurf", df = dfshort, xlabz =  expression(paste("Surface primary productivity", " (g", "m"^"-3", "d"^"-1", ")")), log = FALSE, sample = "Deep", title = "Deep TN - surface PP")

dev.off()

## S7: Surface chla - watdep; surface TN - watdep; full sample TOC - wardep
png("../figures_final/regplot_s7.png", height = 11, width = 10, units = "in", res = 330) #

par(mfrow = c(3,2), mar = c(5,4,2,1))
plotreg(resp = "Chl-a", depvar = "watdepth", df = dfshort, xlabz = expression(paste("Water depth (m)")), log = FALSE, sample = "Surface", title = "Surface Chl-a - water depth")
plotreg(resp = "TOC", depvar = "watdepth", df = dfshort, xlabz =  expression(paste("Water depth (m)")), log = FALSE, sample = "Full sample", title = "Full sample TOC - water depth")
plotreg(resp = "TN", depvar = "watdepth", df = dfshort, xlabz =  expression(paste("Water depth (m)")), log = FALSE, sample = "Surface", title = "Surface TN - water depth")

plotreg(resp = "Phytopigments", depvar = "watdepth", df = dfshort, xlabz =  expression(paste("Water depth (m)")), log = FALSE, sample = "Subsurface", title = "Subsurface phytopigments - water depth")
plotreg(resp = "Phytopigments", depvar = "watdepth", df = dfshort, xlabz =  expression(paste("Water depth (m)")), log = FALSE, sample = "Deep", title = "Deep phytopigments - water depth")

dev.off()

## S8: Surface chla - Log time since dist.; Surface protein - ltsd; subsurface phytopigments - ltst; subsurface TN - ltst



png("../figures_final/regplot_s8.png", height = 8, width = 10, units = "in", res = 330) #JT

par(mfrow = c(2, 2), mar = c(5,4,2,1))
plotreg2(resp = "Chl-a", depvar = "timesincetrawl", df = timedfchla, xlabz = expression(paste("Time since disturbance (d)")),  sample = "Surface", title = "Surface Chl-a - time since disturbance")

plotreg2(resp = "Phytopigments", depvar = "timesincetrawl", df = timedfphyto, xlabz =  expression(paste("Time since disturbance (d)")),  sample = "Subsurface", title = "Subsurface phytopigment - time since disturbance")

plotreg2(resp = "TN", depvar = "timesincetrawl", df = timedftn, xlabz =  expression(paste("Time since disturbance (d)")),  sample = "Subsurface", title = "Subsurface TN - time since disturbance")

plotreg2(resp = "Proteins", depvar = "timesincetrawl", df = timedfprot, xlabz =  expression(paste("Time since disturbance (d)")), sample = "Surface", title = "Surface proteins  - time since disturbance")
...
for(i in seq_along(mods)){
  mods[[i]]$Covar[mods[[i]]$Covar == "log (timesincetrawl)"] <- "log (time since disturbance)"
  mods[[i]]$Covar[mods[[i]]$Covar == "trawling effort"] <- "Fishing effort"
}
...
ggsave("../figures_final/Heatmap_coefficients.png", units = "in", height = 13, width = 16)  
...
ggsave("../figures_final/Heatmap_intercepts.png", units = "in", height = 13, width = 16)  
...
ch = mods[[1]][!(mods[[1]]$Covar %in% c('Season','habitat','trawling effort')),]
ph = mods[[3]][!(mods[[3]]$Covar %in% c('Season','habitat','trawling effort')),]
oc = mods[[4]][!(mods[[4]]$Covar %in% c('Season','habitat','trawling effort')),]
tn = mods[[5]][!(mods[[5]]$Covar %in% c('Season','habitat','trawling effort')),]

hm_chla  <- plothmcoeff2(ch)
hm_phae  <- plothmcoeff2(ph, labz = FALSE)
hm_toc   <- plothmcoeff2(oc)
hm_tn    <- plothmcoeff2(tn, labz = FALSE)

pdf("../figures_final/Heatmaps2.pdf", height = 8, width = 10) #

gradplot2 <- hm_chla + hm_phae + hm_toc + hm_tn

gradplot2 <- gradplot2 +  plot_annotation(tag_levels = "A") +
  plot_layout(guides = "collect", ncol = 2, byrow = TRUE)
