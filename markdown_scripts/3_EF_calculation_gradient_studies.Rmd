...
```{r}
df <- read.csv("../dataset/gradient_studies_flatfile.csv")
...
# # Fix names (first parts can be removed ???)
...
### Eleftherou et al., 1992
```{r}
study = subset(df, df$article_id==183)
vars = unique(study$Response.variable)

# Fishing intensity:
  Fishingunit = "Trawling effort_numerical"
x1 = plotR(data = study, Funit = Fishingunit, var = vars[1], plot = F) # "TOC"
x2 = plotR(data = study, Funit = Fishingunit, var = vars[3], plot = F) # "Chl-a"
x3 = plotR(data = study, Funit = Fishingunit, var = vars[3], plot = F) # "Mean grain size"

# input values into dataframe 
dat = rbind(x1, x2, x3)
dat = dat[,c("var", "r", "n", "V", "SEz")]
tt = Input2 (ID = study, rows = nrow(dat))
G13 = cbind(tt,dat, study = rep("G13", nrow(dat)))

```

### Wang et al., 2021
```{r}
study = subset(df, df$article_id==3)
vars = unique(study$Response.variable)

# Fishing intensity:
  Fishingunit = "hours/km2"
x1 = plotR(data = study, Funit = Fishingunit, var = vars[1], plot = F) # "OM (LOI)"
x2 = plotR(data = study, Funit = Fishingunit, var = vars[8], plot = F) # "Mud content (<63 um)"

# input values into dataframe 
dat = rbind(x1, x2)
dat = dat[,c("var", "r", "n", "V", "SEz")]
tt = Input2 (ID = study, rows = nrow(dat))
G14 = cbind(tt,dat, study = rep("G14", nrow(dat)))
...
```{r}
GradInfo =  rbind(G1,G2,G3, G4, G5, G6, G7, G8, G9, G10, G11, G12, G13, G14)
...
save(file = "../r_objects/Grad_24102022.rda", df)
```