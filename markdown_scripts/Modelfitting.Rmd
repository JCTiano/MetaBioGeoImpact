...
# Edit names
names(df)[c(9:10,14:15,37,44,47,48,49,51,11,38,40,39,3,52,46)] = c("hhabtype","hseason","watdepth","slicedepth","timetrawldays",
...
OCsed <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(hhabtype)-1, weighted=TRUE) # NS; R2=10.41% 
OCsea <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)-1, weighted=TRUE)   # NS; R2=7.66%
...
AIC.rma(M3,M3log) # log slightly better (no-log=8.296817	, log=8.254828)
...
AIC.rma(M4,M4log) # log better (no-log=10.166053, log=9.319881)
...
AIC.rma(M6,M6log) # log better (no-log=6.938558, log=-1.196694)
...
AIC.rma(M7,M7log) # no-log slightly better (no-log=7.688688, log=7.740720)
...
AIC.rma(M8,M8log) # no-log slightly better (no-log=8.474981, log=7.947188)
...
AIC.rma(M9,M9log) # no-log better (no-log=7.742789, log=8.176273)
...
regplot(M6, bg = adjustcolor("red", 0.5), shade = adjustcolor("black", 0.3), grid = T, refline = 0)
...
# Net primary production
M7 <- rma.uni(data=CH, yi=lnRR, vi=VarLnRR, method="REML", mods=~(nppsurf), weighted=TRUE)        # *** R2 = 21.03%
M7log <- rma.uni(data=CH, yi=lnRR, vi=VarLnRR, method="REML", mods=~log(nppsurf), weighted=TRUE)  # *** R2 = 30.64%
AIC.rma(M7,M7log) # log better (no-log=91.88068, log=87.50396)
...
FullModel <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method="ML", weighted=TRUE,
            mods= ~ log(cvel) * log(silbot) * log(shoredist) * slicedepth * hhabtype * hseason ) # 
