...
source("../source_code/FIN_functions_meta_analysis.R")
```
...
```{r}
## JT ?? not sure what why remove objects before loaded files
rm(dfshort)
rm(df)
rm(GradInfo)
...
colnames(gradsel) <- colnames(dfsel) # JT ############################# TODO: fix this  ###################################
...
pdf("../figures_final/Figure2_trueresponseCORRECTED.pdf", height = 8, width = 10) # JT
pleft + p + pright + plot_layout(design = layout)
dev.off()
```
...
# Making sure the variables will be displayed as desired
suball2   <- suball2[order(match(suball2$v, relvar2)),]  # JT can't find relvar2
...
pdf("../figures_final/Figure2SUPPLEMENT_expobs_separated_CORRECTED.pdf", height = 8, width = 10) # JT
pleft + p + pright + plot_layout(design = layout)
dev.off()
```
...
A from Justin: I have grouped TOC and OM (LOI) together. I also added mud and mean grain size. No reason to exclude. 
...
pdf("../figures_final/Figure2_grad_trueresponseJT.pdf", height = 2.5, width = 10) #JT
pleft + p + pright + plot_layout(design = layout)
dev.off()
...
#p1 <- 
#  par(mfrow = c(1,2))
#  plotreg(resp = "TOC", depvar = "heffortnum", df = fishdftoc, xlabz = "trawling effort", log = FALSE)
#  plotreg(resp = "TOC", depvar = "heffortnum", df = fishdftoc, xlabz = "trawling effort", log = FALSE)
...
phaeo_mods <- rbind(modphaeotst, modphaeocur, modphaeowat, modphaeonpp, modphaeosea, modphaeohab) # modphaeotfr left out
...
phyto_mods <- rbind(modphytotst, modphytotfr, modphytocur, modphytowat, modphytonpp, modphytohab) # modphytosea
...
ggsave("../figures_final/Heatmap_coefficientsJT.png", units = "in", height = 13, width = 16)  
...
ggsave("../figures_final/Heatmap_interceptsJT.png", units = "in", height = 13, width = 16)  
...
pdf("../figures_final/Heatmaps2JT.pdf", height = 8, width = 10) #JT
