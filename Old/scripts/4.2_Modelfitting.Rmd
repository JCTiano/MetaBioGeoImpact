...
# Other edits (Sarah suggests not removing these).
...
- df1: immediate trawling impact measurements: =< 1 day impact assessment. => short
- df2: trawling + recovery for all known trawling times.                   => rec
- All: all samples ==> So just df                                          => all
- UN : unknown sampling time since trawl                                   => unk
...
- all is all data, and in this dataset we cannot focus on recovery time.

So I will make three sets of models
- Which shortterm effects can we see.
- Which longer term effects can we see (and how do they recover over time)
- Effects regardless of time since disturbance, focussing on other factors.

```{r subset dataframes to work with}
...
```{r OC short term factors}

sOCdept <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(sslicedepth), weighted = T)
sOChist <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(histfished) , weighted = T)
sOChsea <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hseason)    , weighted = T)
sOChabi <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hhabtype)   , weighted = T)
...
par(mfrow = c(2,2))
boxplot(OCshort$lnRR~OCshort$sslicedepth)
boxplot(OCshort$lnRR~OCshort$histfished)
boxplot(OCshort$lnRR~OCshort$hseason)
boxplot(OCshort$lnRR~OCshort$hhabtype)

```

## Continuous variables

```{r OC short term continuous}

sOCvel      <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(cvel), weighted=TRUE)
sOCfisheff  <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~heffortnum, weighted=TRUE)
sOCdepth    <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~watdepth, weighted=TRUE)
sOCnpp      <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~nppsurf, weighted=TRUE)
sOCoxy      <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~O2bot, weighted=TRUE)
sOCdist     <- rma.uni(data = OCshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~shoredist, weighted=TRUE)
...
rOCdept <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(sslicedepth), weighted = T)
rOChist <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(histfished) , weighted = T)
rOChsea <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hseason)    , weighted = T)
rOChabi <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hhabtype)   , weighted = T)
...
par(mfrow = c(2,2))
boxplot(OCrec$lnRR~OCrec$sslicedepth)
boxplot(OCrec$lnRR~OCrec$histfished)
boxplot(OCrec$lnRR~OCrec$hseason)
boxplot(OCrec$lnRR~OCrec$hhabtype)

```

## Continuous variables

```{r OC recovery term continuous}

rOCvel      <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(cvel), weighted=TRUE)
rOCfisheff  <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~heffortnum, weighted=TRUE)
rOCdepth    <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~watdepth, weighted=TRUE)
rOCnpp      <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~nppsurf, weighted=TRUE)
rOCoxy      <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~O2bot, weighted=TRUE)
rOCdist     <- rma.uni(data = OCrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~shoredist, weighted=TRUE)
...
plot(OCrec$lnRR~OCrec$cvel)
plot(OCrec$lnRR~OCrec$npp)
plot(OCrec$lnRR~OCrec$heffortnum)
...
aOCdept <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(sslicedepth), weighted = T)
aOChist <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(histfished) , weighted = T)
aOChsea <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hseason)    , weighted = T)
aOChabi <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hhabtype)   , weighted = T)
...
par(mfrow = c(2,2))
boxplot(OCall$lnRR~OCall$sslicedepth)
boxplot(OCall$lnRR~OCall$histfished)
boxplot(OCall$lnRR~OCall$hseason)
boxplot(OCall$lnRR~OCall$hhabtype)

```

## Continuous variables

```{r OC alldata term continuous}

aOCvel      <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(cvel), weighted=TRUE)
aOCfisheff  <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~heffortnum, weighted=TRUE)
aOCdepth    <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~watdepth, weighted=TRUE)
aOCnpp      <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~nppsurf, weighted=TRUE)
aOCoxy      <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~O2bot, weighted=TRUE)
aOCdist     <- rma.uni(data = OCall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~shoredist, weighted=TRUE)
...
```{r CHL A short term factors}

sCHdept <- rma.uni(data = CHshort, yi = lnRR, vi = (VarLnRR+0.1), method = "REML", mods = ~(sslicedepth), weighted = T)
sCHhist <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(histfished) , weighted = T)
sCHhsea <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hseason)    , weighted = T)
sCHhabi <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hhabtype)   , weighted = T)
...
par(mfrow = c(2,2))
boxplot(CHshort$lnRR~CHshort$sslicedepth)
boxplot(CHshort$lnRR~CHshort$histfished)
boxplot(CHshort$lnRR~CHshort$hseason)
boxplot(CHshort$lnRR~CHshort$hhabtype)

```

## Continuous variables

```{r Chl a short term continuous}

sCHvel      <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(cvel), weighted=TRUE)
sCHfisheff  <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~heffortnum, weighted=TRUE)
sCHdepth    <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~watdepth, weighted=TRUE)
#sCHnpp      <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~nppsurf, weighted=FALSE)
sCHoxy      <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~O2bot, weighted=TRUE)
sCHdist     <- rma.uni(data = CHshort, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~shoredist, weighted=TRUE)
...
sCHcont <- data.frame("Mods" = c("Current velocity", "Hours fished", "Depth", "O2 bottom", "Dist. shore"),
             "Rsquared" = c(sCHvel$R2, sCHfisheff$R2, sCHdepth$R2, sCHoxy$R2, sCHdist$R2),
             "Pvalue"   = c(sCHvel$QMp, sCHfisheff$QMp, sCHdepth$QMp, sCHoxy$QMp, sCHdist$QMp))
...
```{r CHL A recovery factors}

aCHdept <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(sslicedepth), weighted = T)
aCHhist <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(histfished) , weighted = T)
aCHhsea <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hseason)    , weighted = T)
aCHhabi <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hhabtype)   , weighted = T)
...
par(mfrow = c(2,2))
boxplot(CHall$lnRR~CHall$sslicedepth)
boxplot(CHall$lnRR~CHall$histfished)
boxplot(CHall$lnRR~CHall$hseason)
boxplot(CHall$lnRR~CHall$hhabtype)

```

## Continuous variables

```{r Chl a recovery continuous}

rCHvel      <- rma.uni(data = CHrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(cvel), weighted=TRUE)
rCHfisheff  <- rma.uni(data = CHrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~heffortnum, weighted=TRUE)
rCHdepth    <- rma.uni(data = CHrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~watdepth, weighted=TRUE)
rCHnpp      <- rma.uni(data = CHrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~nppsurf, weighted=TRUE)
rCHoxy      <- rma.uni(data = CHrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~O2bot, weighted=TRUE)
rCHdist     <- rma.uni(data = CHrec, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~shoredist, weighted=TRUE)
...
aCHdept <- rma.uni(data = CHall, yi = lnRR, vi = (VarLnRR+0.1), method = "REML", mods = ~(sslicedepth), weighted = T)
aCHhist <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(histfished) , weighted = T)
aCHhsea <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hseason)    , weighted = T)
aCHhabi <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(hhabtype)   , weighted = T)
...
```{r Chl a all data continuous}

aCHvel      <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~(cvel), weighted=TRUE)
aCHfisheff  <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~heffortnum, weighted=TRUE)
aCHdepth    <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~watdepth, weighted=TRUE)
aCHnpp      <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~nppsurf, weighted=TRUE)
aCHoxy      <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~O2bot, weighted=TRUE)
aCHdist     <- rma.uni(data = CHall, yi = lnRR, vi = VarLnRR, method = "REML", mods = ~shoredist, weighted=TRUE)
...
# Evaluating individual variables
 ...
 - Meta-regressions are run for categorical and continuous variables to assess their stand alone effects and potential inclusion into the final predictive models (see FULL MODEL SELECTION below).
 - Separated into categorical and continuous variables.
...
# TOC
OCdep <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(slicedepth), weighted=TRUE)
OCsed <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(hhabtype)  , weighted=TRUE)
OCsea <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(hseason)   , weighted=TRUE)
OChfi <- rma.uni(data=OC, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(histfished), weighted=TRUE)

# Chl-a
CHdep <- rma.uni(data=CH, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(slicedepth), weighted=TRUE)
CHsed <- rma.uni(data=CH, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(hhabtype)  , weighted=TRUE)   
CHsea <- rma.uni(data=CH, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(hseason)   , weighted=TRUE)
CHhfi <- rma.uni(data=CH, yi=lnRR, vi=VarLnRR, method = "REML", mods=~(histfished), weighted=TRUE)
...
# TOC
OCdep <- rma.uni(data=OCrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(slicedepth), weighted=TRUE)
OCsed <- rma.uni(data=OCrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hhabtype)  , weighted=TRUE)
OCsea <- rma.uni(data=OCrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)
OChfi <- rma.uni(data=OCrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)

# Chl-a
CHdep <- rma.uni(data=CHrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(slicedepth), weighted=TRUE)
CHsed <- rma.uni(data=CHrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hhabtype)  , weighted=TRUE)  
CHsea <- rma.uni(data=CHrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)
CHhfi <- rma.uni(data=CHrec, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)
...
# TOC
OCdep <- rma.uni(data=OCAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(slicedepth), weighted=TRUE)
OCsed <- rma.uni(data=OCAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hhabtype)  , weighted=TRUE)
OCsea <- rma.uni(data=OCAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)
OChfi <- rma.uni(data=OCAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)

# Chl-a
CHdep <- rma.uni(data=CHAll, yi=lnRR, vi=VarLnRR,method = "REML", mods=~(slicedepth), weighted=TRUE)
CHsed <- rma.uni(data=CHAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hhabtype)  , weighted=TRUE)   
CHsea <- rma.uni(data=CHAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)
CHhfi <- rma.uni(data=CHAll, yi=lnRR, vi=VarLnRR, method = "REML",mods=~(hseason)   , weighted=TRUE)
